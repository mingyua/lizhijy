	...
	<div id="box" class="x6" style="height:180px;overflow:hidden">
                ...
                <div class="form-group">
                    ...
                    <div class="field">
					  ...
					  <option value="<?php echo $v['groups']?>"><?php if($v['groups']==1){echo "超级管理员";}else if($v['groups']==2){echo "普通管理员";}else if($v['groups']==3){echo "网站编辑";}?></option>
...
<?php
 if(!empty($_POST['submit'])){
		...
		if($data){
				...
				echo "<script>alert('修改失败！');location.href='admin_user.php'</script>";
	...
	$data=$user->insert(array(
		...
		if($data){
				echo "<script>alert('添加成功！');location.href='admin_user.php'</script>";
		}else{
				echo "<script>alert('添加失败！');location.href='admin_user.php'</script>";
